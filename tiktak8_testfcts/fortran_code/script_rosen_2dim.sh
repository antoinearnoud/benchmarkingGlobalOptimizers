...
gfortran nrtype.f90 stateControl.f90 genericParams.f90 simplex.f90 global.f90 objective_rosen.f90 utilities.f90 minimize.f90 GlobalSearch.f90 -o GlobalSearch

#ifort -m64 -g -debug all -heap-arrays nrtype.f90 stateControl.f90 genericParams.f90 simplex.f90 global.f90 objective_rosen.f90 utilities.f90 minimize.f90 GlobalSearch.f90 -o GlobalSearch
...
chmod u+x config_rosen2.txt
chmod u+x config_rosen10.txt
...
#AMOEBA in 2 dimensions varying points generated/kept
dim=2
...
#overwrite starting point (using 100 random starting points but always the same 100 points for each step in performance profile)
	...
	lineout=37+$dim+$k
	linein=($i-1)*$dim+$k
	sed -i "${lineout}s/.*/`sed -n "${linein}p" init_points_rosen.dat`/g" config_rosen2.txt
	  done

for j in $(seq 1 10); do
#n is number of sobols generated, m is number of sobols kept
	n=20*$j
	m=$n/10

#echo "amoeba dim2 sobol is $n, kept is $m and iter is" $i
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/2, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_rosen2.txt
	./GlobalSearch 0 config_rosen2.txt a
done
...
#BOBYQA in 2 dimensions varying points generated/kept
...
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/2, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_rosen2.txt
	./GlobalSearch 0 config_rosen2.txt b
done
