...
contains
    ...
    SUBROUTINE dfovec(n, mv, x, v_err)
        ...
        v_err=SIN(3.141592653589793D0*x(1)*3.0D0)**2 + &
                (SIN(3.141592653589793D0*x(n)*2.0D0)**2+1.0D0)*(x(n)-1.0D0)**2       
        DO i=1,n-1
            v_err=v_err+(SIN(3.141592653589793D0*x(i+1)*3.0D0)**2+1.0D0)*(x(i)-1.0D0)**2
        ENDDO
        ...
        !v_err=x(1)**2-10*cos(2*3.141592653589793D0*x(1))
        !do i=2,n
        !    v_err=v_err+x(i)**2-10*cos(2*3.141592653589793D0*x(i))
        !enddo
