PROGRAM GlobalSearch
    ...
    USE utilities
    USE minimize
    USE OBJECTIVE, only : objFun, dfovec, initial0
    ...
    ! For printing minimum (aa)
    !allocate(global_min_x(p_nx))
    ...
    DO WHILE(currentState .NE. p_exitState)
        ...
        IF (driver /= newDriver) THEN
            ...
            driver = newDriver
            deallocate(trial)
            allocate(trial(p_nx, p_qr_ndraw-sobdrop))
        END IF
        ...
        SELECT CASE (currentState)
            ...
            CASE (3)
                ...
                IF (complete) THEN
                    ...
                    ! done.
                    print*, "Done evaluating objective function at Sobol points."
            ...
            CASE (4)
                ...
                print*, "starts search for minimum"
            ...
            CASE (5)
                IF (isInit(seqNo)) THEN
                    ...
                    ! points.
                    print*,"looking for points from which minimization has not been undertaken"
                    call findMissingSearch(complete)
                    IF (complete) THEN
                        print*,"all points have been optimized. going to last optimization."
                    ...
                    ELSE
                        ...
                        IF(setState(4,seqNo)) THEN
                            print*,"some points have not been minimized"
                            cycle
                        END IF
                    END IF
                ELSE
            ...
            CASE (6)
                ...
                print*, "sort the Sobol points (and keep the best n)"
                IF(isInit(seqNo)) THEN
                    call setupSobol()
                ELSE
                    call statePause(currentState)
            ...
            CASE (8)
                IF (isInit(seqNo)) THEN
                    ...
                    ! and store them.
                    print*,"Looking for Sobol points which have not been evaluated..."
                    ...
                    ELSE
                        ...
                        IF(setState(3,seqNo)) THEN
                            print*, "some Sobol ponts have not been evaluated. now evaluating them."
                            cycle
                        END IF
                    END IF
                ELSE
    ...
    open(unit = 111, file = "monteCarloResults.dat", position = "append", STATUS='unknown')
    !we write out algorithm indicator (writes 1 or 0 where 1=amoeba and 0=bobyqa, then we write function evaluation counter, cputime, number of sobols that are kept to start local searches, and number of sobols overall that are drawn.
    !also want to write out the first starting value to know which iteration was written out...
    write(111,*) alg, fe_counter, cpuTime, p_maxpoints, p_qr_ndraw, p_nx, p_init(1)
    close(111)
    !deallocate(global_min_x)
...
contains
    SUBROUTINE solveAtSobolPoints(seqNo, complete)
	    ...
	    write(*, *) seqNo," solving sobol point ",whichPoint
            ...
            call myopen(unit=fileDesc, file='sobol.dat',status='old', IOSTAT=openStat, ACTION='read')
            read(fileDesc,*) trial
            call myclose(fileDesc)
...
7452    format(2i10, 200f40.20)
    ...
    SUBROUTINE completeSearch (seqNo, algor, whichPoint, evalParam)
        ...
        !output result
        currentBest = getBestLine(openStat)  ! currentBest is a vector with evaluation of current best point in 3rd row, and best point from 4th row on
        i = INT(currentBest(2))
        ...
        write(fileDesc,7453) seqNo, whichPoint, evalBack   ! evalBack = evalParam before the search
        call myclose(fileDesc)
...
7453    format(2i10, 200f40.20)
    ...
    SUBROUTINE lastSearch(seqNo)
        ...
        implicit none
        INTEGER(I4B), INTENT(IN) :: seqNo
        INTEGER(I4B) :: i, openStat, dummy, half_maxeval
        REAL(DP), DIMENSION(p_nx) :: evalParam
        REAL(DP) :: rhobeg, rhoend_lastsearch, fn_val
	...
	print*, "Best point so far gives a value for f of: ", objFun(evalParam)

        rhobeg  = minval(p_bound(:,2)-p_bound(:,1))/10.0_DP
        rhoend_lastsearch  = 1.0D-8/4.0_dp
        half_maxeval = p_maxeval/2
        call bobyqa_h(p_nx,p_ninterppt,evalParam,p_bound(:,1),p_bound(:,2),rhobeg,rhoend_lastsearch, &
                     & p_iprint,half_maxeval,p_wspace,p_nmom)
	...
	print*, '=============================================='
	print*, 'argument at optimum (argmax): ', bestValue
	print*, 'optimum function value: ', bestParam

        open(unit = 111, file = "monteCarloResultsMin.dat", position = "append", STATUS='unknown')
...
8111    format(200f40.20) ! (aa)
        ...
        !write(112,8112) alg, fe_counter, cpuTime, p_maxpoints, p_qr_ndraw, p_nx, &
        !& rhoend_dfls, p_maxeval, rhoend_lastsearch, half_maxeval, p_tolf_amoeba, p_itmax_amoeba, &
        !& p_init(1), &
        !& bestValue, bestParam
        !close(112)
8112 format(2I30,f40.20,3I30,f40.20,I30,f40.20,I30,f40.20,I30,200f40.20) ! (aa)
...
492     call myclose(fileDesc)
        count1 = count1-1
        ilo=minloc(fval_in(1:count1),1)   ! finds the minimum. minloc(array,dim) where dim is dimension of array
        y(1) = DBLE(seqN(ilo))
...
7452    format(2i10, 200f40.20)
    ...
    SUBROUTINE setupSobol()
        ...
        REAL(DP), DIMENSION(p_qr_ndraw-sobdrop) :: fval
        INTEGER(I4B), DIMENSION(p_qr_ndraw-sobdrop) :: fval_init_index, whichPoints

        call myopen(UNIT=fileDesc, FILE='sobolFnVal.dat', STATUS='old', IOSTAT=ioStat, ACTION='read')
        DO i=1,p_qr_ndraw-sobdrop  ! aa: number of sobol points?
        ...
        !Sort the function values. fval_init_index(1) gives the smallest
        !element in fval, fval_init_index(2) gives the next smallest, etc.
        CALL indexx(p_qr_ndraw-sobdrop,fval,fval_init_index)
...
269     format(i10)
    ...
    SUBROUTINE findMissingSobol(allFinished)
        ...
        LOGICAL, DIMENSION(p_qr_ndraw-sobdrop) :: solvedPoints
            ...
            IF (isWarm) THEN
                ...
                SELECT CASE (arg2)
                    ...
                    CASE ('b')
                        alg = 0
                    CASE default
                        write(*,*) "Invalid algorithm provided"
                        STOP 0
                        !alg = p_default_alg
                END SELECT
            ELSE
