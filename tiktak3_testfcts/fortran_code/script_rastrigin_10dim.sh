...
#overwrite starting point (using 100 random starting points but always the same 100 points for each step in performance profile)
	...
	lineout=37+$dim+$k
	linein=($i-1)*$dim+$k
	sed -i "${lineout}s/.*/`sed -n "${linein}p" init_points_rastrigin.dat`/g" config_rastrigin10.txt
	  done
...
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/10, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_rastrigin10.txt
	./GlobalSearch 0 config_rastrigin10.txt a
done
...
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/10, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_rastrigin10.txt
	./GlobalSearch 0 config_rastrigin10.txt b
