MODULE genericParams
    ...
    INTEGER(I4B) :: p_nx, p_nmom, p_iprint=1, p_maxeval, p_ninterppt
    ...
    REAL(DP), PARAMETER :: rhoend_dfls = 1.0D-3
...
CONTAINS
    SUBROUTINE initialize(seqNu, update, config)
        ...
        IF (PRESENT(config)) THEN
            ...
            call myopen(UNIT=fileDesc, FILE='internalConfig.dat', STATUS='replace', IOSTAT=openStat, ACTION='write')
            write(fileDesc,*) p_nx, p_nmom, p_maxeval, p_qr_ndraw, p_maxpoints-1, p_searchType, p_lotteryPoints
            DO i=1,p_nx
