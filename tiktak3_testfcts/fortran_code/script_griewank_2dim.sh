...
#overwrite starting point (using 100 random starting points but always the same 100 points for each step in performance profile)
	...
	lineout=37+$dim+$k
	linein=($i-1)*$dim+$k
	sed -i "${lineout}s/.*/`sed -n "${linein}p" init_points_griewank.dat`/g" config_griewank2.txt
	  done
...
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/2, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_griewank2.txt
	./GlobalSearch 0 config_griewank2.txt a
...
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/2, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_griewank2.txt
	./GlobalSearch 0 config_griewank2.txt b
done
