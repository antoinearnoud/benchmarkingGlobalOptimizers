...
gfortran nrtype.f90 stateControl.f90 genericParams.f90 simplex.f90 global.f90 objective_rastrigin.f90 utilities.f90 minimize.f90 GlobalSearch.f90 -o GlobalSearch

#ifort -m64 -g -debug all -heap-arrays nrtype.f90 stateControl.f90 genericParams.f90 simplex.f90 global.f90 objective_rastrigin.f90 utilities.f90 minimize.f90 GlobalSearch.f90 -o GlobalSearch
...
chmod u+x config_rastrigin2.txt
chmod u+x config_rastrigin10.txt
...
#overwrite starting point (using 100 random starting points but always the same 100 points for each step in performance profile)
	...
	lineout=37+$dim+$k
	linein=($i-1)*$dim+$k
	sed -i "${lineout}s/.*/`sed -n "${linein}p" init_points_rastrigin.dat`/g" config_rastrigin2.txt
	  done
...
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/2, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_rastrigin2.txt
	./GlobalSearch 0 config_rastrigin2.txt a
done
...
#overwrite number of Sobol points generated / kept
	sed -i "28s/.*/2, 1, -1, ${n[@]}, ${m[@]}, 0, 10/g" config_rastrigin2.txt
	./GlobalSearch 0 config_rastrigin2.txt b
done
