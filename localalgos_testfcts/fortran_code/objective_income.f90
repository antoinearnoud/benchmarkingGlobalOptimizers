module OBJECTIVE
    ...
    use nrtype
    use myparams  ! global params: counters
    ...
    implicit none
    PRIVATE
    PUBLIC objFun, dfovec, gradf, initial0, GetData, getPenalty
...
contains
    ...
    elseif(funcloop == 5) Then 
      objFun = objFun_income(x)
 ...
 SUBROUTINE dfovec(n, mv, x, v_err)
      ...
      if(funcloop /= 5) then
        ...
        !FIXME
        if(funcloop==5) then
        call dfovec_income(n, mv, x, v_err)  
        endif
...
! this added in order to have income process in same files than test functions
...
implicit none
    ...
    REAL(DP),DIMENSION(p_nx),INTENT(IN) :: theta
    REAL(DP),DIMENSION(p_nx)            :: pencons 
    REAL(DP)                            :: objFun0(1,1), objFun_income, penalty
    ...
    objFun_income = objFun0(1,1)+getPenalty(theta)

END FUNCTION objFun_income
...
! -------------------------------------------------------------------------
SUBROUTINE dfovec_income(np, nm, x, Fnout)
