MODULE minimize
    !This module contains minimization routines Nelder-Mead (amoeba), dfnls (bobyqa) and dfpmin.
    use nrtype
    !use utilities
    ...
    SUBROUTINE amoeba(p, y, ftol, func, iter, ITMAX)
        ...
        INTERFACE
            FUNCTION func(x)
                use genericParams ! might put this somewhere else (just need acces to p_nx) since in the function ObjFun there is actually no call to genericParams
                use nrtype
                IMPLICIT NONE
	    ...
	    !INTEGER(I4B), PARAMETER :: ITMAX = 5000      ! commented out.
        INTEGER(I4B), INTENT(IN) :: ITMAX             ! maximum number of iterations allowed. If above, returns best result so far.
    ...
    CONTAINS
        ...
        SUBROUTINE amoeba_private
            ...
            iter=0              ! initialize iterations to 0
            psum(:)=sum(p(:,:),dim=1)
            do
                ilo=iminloc(y(:))
                ihi=imaxloc(y(:))
                ...
                ! I commented below so it keeps runnning even if the original Amoeba would have stopped because simplex super small (?)
                !if (rtol < ftol) then
                      !print*, 'ftol', ftol
                      !print*, 'rtol', rtol
                      !print*, 'in amoeaba, loop to return'
                    ! call swap(y(1),y(ilo))
                  ! call swap(p(1,:),p(ilo,:))
                  ! RETURN
                ! end if
                !if (iter >= ITMAX) call nrerror('ITMAX exceeded in amoeba')
                if (iter >= ITMAX) then                     ! I addeed this instead of previous line  so it returns whatever it got
                ...
                ytry=amotry(-1.0_dp)  
                iter=iter+1           ! amotry uses ONE function evaluation
                if (ytry <= y(ilo)) then
                    ytry=amotry(2.0_dp)
                    iter=iter+1       ! amotry uses ONE function evaluation
                else if (ytry >= y(inhi)) then
                    ysave=y(ihi)
                    ytry=amotry(0.5_dp) ! amotry uses ONE function evluation
                    iter=iter+1
                    if (ytry >= ysave) then
                        p(:,:)=0.5_dp*(p(:,:)+spread(p(ilo,:),1,size(p,1)))
                        do i=1,ndim+1
                            !if (i /= ilo) y(i)=func(p(i,:))
                            if (i /= ilo) then      ! added by AA TK. Variation over line above to keep better track of number of function evaluations
                                y(i)=func(p(i,:))   ! added by AA TK
                                iter=iter+1         ! added by AA TK
			                endif                   ! added by AA TK
                        end do
                        !iter=iter+ndim 		    ! commented by AA TK. uses the incrementation in the do loop instead
                        psum(:)=sum(p(:,:),dim=1)
                    end if
                end if
            end do
        ...
        FUNCTION amotry(fac)
          ! uses ONE function evaluation
            ...
            ytry=func(ptry) ! call the function
            if (ytry < y(ihi)) then
                y(ihi)=ytry
...
! DFPMIN
    ! DFPMIN
    ! dfpmin for function called ObjFun (function defined in objective.f90)
    SUBROUTINE dfpmin(p,gtol,iter,fret, ITMAX) ! p: initial points and argmin, gtol: tolerance for gradient, iter: nb iterations, fret: minf, ITAMX: max number of iterations
    USE nrtype
    USE nrutil, ONLY : nrerror,outerprod,unit_matrix,vabs
    USE genericParams, only: p_nx
    USE objective
    !USE nr, ONLY : lnsrch
    IMPLICIT NONE
    ...
    REAL(DP), DIMENSION(p_nx),  INTENT(INOUT)   :: p
    ! INTERFACE NOT USEFUL ANYMORE SINCE CHANGED FUNCTION FOR ObjFun
   ! INTERFACE
   !   FUNCTION func(p)
   !   !use genericParams
   !   implicit none
   !   REAL(DP),DIMENSION(p_nx),  INTENT(IN) :: p
   !   REAL(DP)            :: func
   !   END FUNCTION func
  !BL
    !  FUNCTION dfunc(p)
   ...
   ! ADDED This
    !    INTERFACE
    !      SUBROUTINE lnsrch(xold,fold,g,p,x,f,stpmax,check, iter) ! added iter
    ...
    !
    !     INTEGER(I4B),     INTENT(OUT)   :: iter ! added
    !
    !   !INTERFACE
    !   ! FUNCTION func(x)
    !   ! USE nrtype
    ...
    REAL(DP),     PARAMETER     :: STPMX=100.0D0,EPS=epsilon(p),TOLX=4.0D0*EPS
    INTEGER(I4B)      :: its
    LOGICAL           :: check
    ...
    INTEGER(I4B)  :: iter_lnsrch  ! added this

    iter = 0                      ! added this

    fp=objFun(p)
    g=gradf(p)
    iter = iter + 2       ! added this
    ...
    do its=1,ITMAX
  !     if (counter >= ITMAX) RETURN    ! added this. AA-TK
       if (iter >= ITMAX) RETURN    ! added this. AA-TK
!      iter=its         ! commented this
!      call lnsrch(p,fp,g,xi,pnew,fret,stpmax,check,objFun)
      call lnsrch(p,fp,g,xi,pnew,fret,stpmax,check, iter_lnsrch) ! added iter_lnsrch. AA-TK
      iter = iter + iter_lnsrch     ! added this
      fp=fret
      xi=pnew-p
      p=pnew
      if (maxval(abs(xi)/max(abs(p),1.0D0)) < TOLX) RETURN
      dg=g
      g=gradf(p)
      iter= iter + 1        ! added this
      den=max(fret,1.0D0)
      if (maxval(abs(g)*max(abs(p),1.0D0)/den) < gtol) RETURN
      dg=g-dg
    ...
    !call nrerror('dfpmin: too many iterations')    ! changed this into RETURN. AA-TK
    RETURN              ! I added this. AA-TK
